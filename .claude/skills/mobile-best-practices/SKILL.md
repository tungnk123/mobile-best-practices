...
|----------|-----------|-------|
| `android` | 60+ | Jetpack Compose, Material3, Coroutines, Hilt, Room, Navigation, Gradle |
| `ios` | 35+ | SwiftUI, Combine, async/await, Actors, NavigationStack |
| `flutter` | 34+ | Widgets, BLoC/Riverpod, Dart, GoRouter, Slivers |
| `react-native` | 35+ | Hooks, React Navigation, Reanimated, FlatList |

### Available Stacks (--stack)

| Stack | Maps To | Focus |
|-------|---------|-------|
| `compose` / `jetpack-compose` | Android | Jetpack Compose, Material3 |
| `material3` / `hilt` / `room` / `kotlin` | Android | Specific Android tech |
| `swiftui` / `combine` / `uikit` / `swift` | iOS | Apple development |
| `flutter` / `dart` / `bloc` / `riverpod` | Flutter | Flutter/Dart ecosystem |
| `react-native` / `rn` / `hooks` / `redux` | React Native | RN ecosystem |
...
| `--stack` / `-s` | Search by tech stack | `--stack compose` |
| `--filter-platform` / `-fp` | Filter any domain by platform | `--domain library --filter-platform android` |
| `--persist` | Save architecture blueprint | `--persist -pn "MyApp"` |
| `--project-name` / `-pn` | Project name for blueprint | `-pn "ShopApp"` |
| `--page` | Page-specific blueprint override | `--page "checkout"` |
...
### Stack Search: "Get Compose-specific best practices"

```bash
# Search by tech stack
python3 .claude/skills/mobile-best-practices/scripts/search.py "state management" --stack compose -n 3
python3 .claude/skills/mobile-best-practices/scripts/search.py "navigation" --stack swiftui -n 3
python3 .claude/skills/mobile-best-practices/scripts/search.py "routing" --stack flutter -n 3
python3 .claude/skills/mobile-best-practices/scripts/search.py "performance" --stack react-native -n 3
```

### Generate Blueprint: "Create architecture document"

```bash
# Generate MASTER.md blueprint
python3 .claude/skills/mobile-best-practices/scripts/search.py "e-commerce android" --persist -pn "ShopApp"

# Generate page-specific override
python3 .claude/skills/mobile-best-practices/scripts/search.py "checkout payment" --persist -pn "ShopApp" --page "checkout"

# Creates:
# architecture-blueprint/
# ├── MASTER.md              # Global architecture blueprint
# └── pages/
